...
from seeded_lda_feature import SeededLDAFeature
...
def filter_features(feature_name_types, author_ids, all_features, neu_labels, is_testing_data, filtered_feature_name_types=[]):
	if not is_testing_data:		
		filtered_feature_name_types = feature_name_types
		feature_vectors = {}
		labels = []
		...
		for idx in feature_vectors.keys():
			...
			if abs(p[0]) < 0.01 or p[1] > 0.1:
				filtered_feature_name_types[idx] = (None,None)
		for author_id in author_ids:
			feature_values = all_features[author_id]
			for idx in range(0, len(feature_values)):
				if filtered_feature_name_types[idx] == (None,None):
					feature_values[idx] = None
			new_feature_values = []
			for feature_value in feature_values:
				if feature_value != None:
					new_feature_values.append(feature_value)
			all_features[author_id] = new_feature_values
		new_filtered_feature_name_types = []
		for name in filtered_feature_name_types:
			if name != (None,None):
				new_filtered_feature_name_types.append(name)
		return new_filtered_feature_name_types, all_features 
	else:
		for author_id in author_ids:
			feature_values = all_features[author_id]
			new_feature_values = []
			for idx in range(0, len(feature_values)):
				if(feature_name_types[idx] in filtered_feature_name_types):
					new_feature_values.append(feature_values[idx])
			all_features[author_id] = new_feature_values	
		return filtered_feature_name_types, all_features

def process_training_data(disk, training_data, unigrams, bigrams, bag_of_words, bag_of_bigrams, topic_model_feature, seeded_lda_feature, is_testing_data=False, filtered_feature_name_types=[]):
    ...
    if disk == 0:
        ...
        if seeded_lda_feature is not None:
            print title, "data seeded lda features extracting..."
            seeded_lda_feature.get_seeded_lda_features(author_ids, all_features)
            print title, "data seeded lda features extracted"
        ...
        if topic_model_feature:
            for idx in range(0, constants.TOPICS):
                feature_name_types.append(('topic_model_' + str(idx), 'numeric'))

        if seeded_lda_feature:
            for idx in range(0, constants.SEEDED_TOPICS):
                feature_name_types.append(('seeded_lda_' + str(idx), 'numeric'))

	if constants.FILTER_FEATURE == "TRUE":	
		if not is_testing_data:
			(feature_name_types, all_features) = filter_features(feature_name_types, author_ids, all_features, neu_labels, is_testing_data)
			filtered_feature_name_types = feature_name_types
		else:
			(feature_name_types, all_features) = filter_features(feature_name_types, author_ids, all_features, neu_labels, is_testing_data, filtered_feature_name_types)
    ...
    if not is_testing_data:
	return all_features, neu_labels, filtered_feature_name_types
...
if __name__ == "__main__":
    ...
    print "Seeded LDA feature..."
    seeded_lda_feature = SeededLDAFeature()
    #seeded_lda_feature = None
    print "Seeded LDA feature done"
    
    all_features, neu_labels, filtered_feature_name_types = process_training_data(0, training_data, unigrams, bigrams, bag_of_words, bag_of_bigrams, topic_model_feature, seeded_lda_feature)

    print "Testing data loading..."
    test_data = CsvReader(constants.TEST_DATA_FILE, True)
    print "Testing data loaded."

    features_test, neu_labels_not_used = process_training_data(0, test_data, unigrams, bigrams, bag_of_words, bag_of_bigrams, topic_model_feature, seeded_lda_feature, True, filtered_feature_name_types)
